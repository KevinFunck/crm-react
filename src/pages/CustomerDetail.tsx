...
export default function CustomerDetail({ customers, setCustomers }: Props) {
  ...
  // ---------------------------
  // Hooks (always call them in the same order)
  // ---------------------------
  ...
  // ---------------------------
  // If customer not found, show message
  // ---------------------------
  ...
  const handleAddContact = () => {
    ...
    const updatedCustomers = customers.map(c =>
      c.id === customer.id
        ? {
            ...c,
            contacts: [...c.contacts, { id: Date.now().toString(), ...newContact }],
          }
        : c
    );
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <Dialog.Panel className="bg-gray-900 p-6 rounded-2xl shadow-lg max-w-md w-full text-white">
          <Dialog.Title className="text-xl font-bold mb-4">Add Contact Person</Dialog.Title>
